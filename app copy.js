...
function renderGlossary() {
  ...
  // -------------------------------------------------------------
  // 1. PROCESAMIENTO PARA GLOSARIO DE NOMBRES Y PALABRAS
  // -------------------------------------------------------------
  ...
  // -------------------------------------------------------------
  // 2. FUNCIÓN AUXILIAR DE CONSTRUCCIÓN PARA NOMBRES / PALABRAS
  // -------------------------------------------------------------
  ...
  // Renderizar secciones de Nombres y Palabras
  buildGlossarySection(namesMap, namesContainer, namesNav, "names");
  buildGlossarySection(wordsMap, wordsContainer, wordsNav, "words");

  // -------------------------------------------------------------
  // 3. PROCESAMIENTO Y RENDERIZADO DEL GLOSARIO DE CATEGORÍAS
  // -------------------------------------------------------------
  if (!categoriesContainer) return;

  const sortedCategories = [...WORD_DATA].filter(c => c && c.category).sort((a, b) => 
  ...
  // Navegación A-Z de categorías
  const availableCatLetters = new Set(
    sortedCategories.map(cat => normalize(cat.category).charAt(0).toUpperCase())
  );
  ...
  if (categoriesNav) {
    ...
    alphabet.forEach(letter => {
      ...
      if (availableCatLetters.has(letter)) {
        ...
        btn.style.color = "var(--primary, #2b6cb0)";
        btn.addEventListener("click", () => {
          const headerEl = $(`cat-letter-${letter}`);
          if (headerEl) {
            headerEl.scrollIntoView({ behavior: "smooth", block: "start" });
          }
        });
  ...
  sortedCategories.forEach(mainCat => {
    const normName = normalize(mainCat.category);
    const firstLetter = normName.charAt(0).toUpperCase();

    // Encabezados por letra A-Z
    if (firstLetter !== currentCatLetter) {
      currentCatLetter = firstLetter;

      const headerContainer = document.createElement("div");
      headerContainer.id = `cat-letter-${currentCatLetter}`;
      headerContainer.style.display = "flex";
      headerContainer.style.alignItems = "center";
      headerContainer.style.gap = "12px";
      headerContainer.style.margin = "16px 0 6px 0";
    ...
    if (mainCat.fortext) {
      catTitleText += ` (fortext: ${mainCat.fortext})`;
    }
    ...
    if (subCats.length > 0) {
      ...
      subCats.forEach(sub => {
        ...
        if (sub.fortext) {
          subTitleText += ` (fortext: ${sub.fortext})`;
        }
        ...
        // Lista de palabras de la subcategoría
        if (Array.isArray(sub.words) && sub.words.length > 0) {
          ...
          wordsDiv.textContent = `Palabras: ${formattedWords}`;
